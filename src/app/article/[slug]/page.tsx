...
const SingleArticle: React.FC<TSingleArticleProps> = async ({ params }) => {
  ...
  try {
    const res = await axios.get(
      ...
      {
        headers: {
          Authorization: session.token ? `Token ${session.token}` : undefined,
        },
      },
    );
    ...
    if (data.status === 200) {
      ...
      return (
        <>
          <SingleArticleHeader
            ...
            isCurrentUser={session.username === article.author.username}
          />
          <p className="py-5 px-4 md:px-10 lg:px-14 text-lg text-justify">{article.body}</p>
          <TagList tagList={article.tagList} className="py-5 px-4 md:px-10 lg:px-14" />
          ...
          {session.token ? (
            <>
              <CommentForm
                avatar={<AvatarImg username={session.username as string} className="h-7 w-7" />}
            ...
            <div className="w-fit mx-auto mb-5">
              <Link href="/login" className="text-green-custom hover:underline">
                Sign in
              </Link>{" "}
              or{" "}
              <Link href="/register" className="text-green-custom hover:underline">
                sign up
              </Link>{" "}
      ...
      return (
        <h1 className="text-rose-500 w-fit mx-auto">
          <FormattedErrors data={data.data} className="ml-5 capitalize" />
        </h1>
      );
    }
  } catch (err) {
