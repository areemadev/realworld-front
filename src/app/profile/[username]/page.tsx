...
export default async function ProfilePage(props: TProfilePageProps) {
  ...
  const { page, offset, tag, limit, author, favorited, feed } = getParams(props.searchParams);
          ...
          <div className="min-w-full py-2 px-4 md:px-10 lg:px-14 mt-10">
            <ArticleHeader
              choices={[
                {
                  name: "My Articles",
                  query: `author=${author ? author : props.params.username}`,
                },
                {
                  name: "Favorited Posts",
                  query: `favorited=${favorited ? favorited : props.params.username}`,
                },
              ]}
            />
