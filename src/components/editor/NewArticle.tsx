...
export const NewArticle: React.FC<TEditorProps> = ({
  ...
  return (
    <>
      {state.isError && <FormattedErrors data={state.errors} className="ml-5" />}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Textarea
                      ...
                      onKeyUp={(e) => {
                        if (e.key === "Enter") {
                          if (
                            !state.tagList.find((tag) => tag === (field.value?.trim() as string)) &&
                            (field.value?.trim() as string) !== ""
                          ) {
                            state.tagList.push(field.value?.trim() as string);
                            form.reset({
                  ...
                  <FormMessage />
                  <FormDescription>hint: after each tag press enter key</FormDescription>
                </FormItem>
              )}
            />
          )}
                ...
                >
                  <X
                    ...
                    onClick={() => {
                      setState((prevState) => ({
                        ...prevState,
                        tagList: state.tagList.filter((e) => e !== ele),
                      }));
                    }}
                  />
                  {ele}
