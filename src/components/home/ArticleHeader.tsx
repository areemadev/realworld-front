...
export const ArticleHeader: React.FC<TArticleHeader> = ({ choices }) => {
  ...
  const query: string = path.startsWith("/profile")
    ? search.value && search.value[0] != "page" && search.value[0] != "offset"
      ? search.value.join("=")
      : `author=${path.split("/")[2]}`
    : search.value && search.value[0] != "page" && search.value[0] != "offset"
      ? search.value.join("=")
        ...
        {choices.map((choice, i) => (
          <Link
            className={cn(
              ...
              query === choice.query && " border-b-green-custom text-green-custom",
            )}
            key={i}
