...
export const PaginationComponent: React.FC<TPaginationComponents> = ({
  ...
  const paginationLimit: number = Math.ceil(articlesCount / 10);
  ...
  const paginationElements: number[] = [];
  const paginationEnd: number = page > 10 ? page : paginationLimit < 10 ? paginationLimit : 10;
  const paginationStart: number = page > 10 ? page - 9 : 1;
