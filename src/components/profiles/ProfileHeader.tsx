      ...
      <div className="w-full md:w-4/5 lg:w-3/5 mx-auto flex flex-col align-middle items-center justify-center gap-2">
        <div className="relative w-24 h-24 rounded-full">
          <Image
            ...
            fill
            className={cn("rounded-full object-fit", !profile.image && "object-cover border")}
          />
        </div>
        <h1 className=" font-bold text-2xl w-fit h-fit">{profile.username}</h1>
        <p className="w-fit h-fit font-light text-gray-400">{profile.bio}</p>
        {session.username ? (
